...
from service import MenuManagementService
...
def create(event, _):
    ...
    menu: Menu = request_mapper.map(request_body)
...
def list(event, _):
    ...
    menus: list = service.find_all_metadata_by_merchant_id(merchant_id)
...
def get(event, _):
    ...
    menu = service.find_by_merchant_and_menu(merchant_id, menu_id)
